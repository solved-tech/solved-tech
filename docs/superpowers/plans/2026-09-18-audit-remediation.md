...
**H4 (FAQ half) applied.** The four audit questions now carry answers built only from claims the site already made — diagnosis before a fix is promised, scope and estimate before work starts, short review stages, handover with documentation, and ongoing work scoped and estimated separately. Two guards moved with the content: the FAQ test now asserts no unfilled answers in any state, and the proof/FAQ render test asserts the FAQ section renders while the proof section still does not (no case study exists yet). The FAQ section appears on the homepage automatically as a result.

**H7 applied.** All three service pages are `approved: true` in `servicePages`; the founders approved the copy as written. They stay `noindex` until launch, because the shells keep the meta and `injectPrerender` only strips it when the site is launched — once launched they become indexable and enter the sitemap and `og:` metadata automatically.

**H2 complete, H5 host decided.** The privacy notice now carries real values: operated by "Solved Tech Limited, 31 Glendale Gardens, Wembley, England, HA9 8PR", last updated 18 September 2026, calls and WhatsApp carried by EE, enquiry correspondence kept for 24 months. The token test now asserts no tokens in any state. One disclosure gap remains and is recorded rather than papered over: the mailbox is not set up yet, so instead of naming a processor the notice says "Email is processed by our email hosting provider" — replace that phrase with the provider name once mail is live, along with the MX records. H5 decided: the site stays on GitHub Pages at `/solved-tech/`, so `base` is unchanged, no `public/CNAME` is added, and the launch origin is `https://solved-tech.github.io`.

**Added beyond the plan (founder request, 18 September 2026):** a favicon set — `public/favicon.svg` (the "S" mark on the site's dark ground, geometry transcribed from `solved-tech-logo-dark.svg`), `public/favicon-32.png` and `public/apple-touch-icon.png` (rasterised from that SVG). All five shells link them with root-relative paths (`/favicon.svg`), so Vite applies `base` in both dev and build; `%BASE_URL%` was tried first and produced a doubled base under `vite dev`. Guarded by six tests: one per shell for the links and one for the asset files. Suite is now 132 tests.
