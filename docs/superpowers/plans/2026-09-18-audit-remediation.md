...
**Task 15 is still blocked.** Step 1 checks after H1 and H4(FAQ): `grep -c '\[\[' src/content.ts` → 4 (all privacy: lines 212, 217, 236, 242), `placeholder: false`, `approved: false` ×3, `caseStudies: []`, and no `public/brand/solved-tech-social.png`. Outstanding: H2 (retention period, carrier provider, email provider, notice date, data-protection sign-off), H5 (production origin, host confirmation, 1200×630 social preview) and H7 (service-page approval); plus the case-study half of H4, and optionally H3 (Remus portrait ≥680 px) and H6 (analytics vendor). Never invent these.
